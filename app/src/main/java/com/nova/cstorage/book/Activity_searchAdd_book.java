...
public class Activity_searchAdd_book extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.book_searchadd);
        ...
        imagebutton = findViewById(R.id.btn_addbs_img);

        input_booktitle = (EditText) findViewById(R.id.book_adds_title);
        input_bookauthor = (EditText) findViewById(R.id.book_adds_author);
        input_bookreview = (EditText) findViewById(R.id.book_adds_review);

//        arraySearchBook = (ArrayList<BookSearchData>)getIntent().getSerializableExtra("bookItem");
...
//        try {
        if (!(bookSearchData == null)){
            ...
            bookSearchData.setTitle(title);
            bookSearchData.setAuthor(author);
            bookSearchData.setBook_pic(bookP);

            String imageUrl = bookSearchData.getBookpic();
            Log.e("바인드데이터", "타이틀" + bookSearchData.getTitle());
            if (bookSearchData.getBookpic().isEmpty()) {
                imagebutton.setImageResource(R.drawable.book);
            } else {
                Picasso.get().load(imageUrl).into(imagebutton);
            }

            input_booktitle.setText(Html.fromHtml(title));
            input_bookauthor.setText(Html.fromHtml(author));
        }
...
//        }
        ...
        Button btn_addbs_save = (Button) findViewById(R.id.btn_addbs_save);
        Button btn_addbs_cancle = (Button) findViewById(R.id.btn_addbs_cancle);
        ImageButton btn_addbs_img = (ImageButton) findViewById(R.id.btn_addbs_img);


        sharedPreferences = getSharedPreferences(SharedPreFile, MODE_PRIVATE);

        btn_addbs_save.setOnClickListener(this);
        btn_addbs_cancle.setOnClickListener(this);
        btn_addbs_img.setOnClickListener(this);
    }

    @Override
    public void onClick(View v) {
        if (v.getId() == R.id.btn_addbs_save) {
            ...
            if (book_review.equals("") && book_author.equals("") && book_title.equals("")) {//빈값이 넘어올때의 처리
                ...
                sendBitmap = BitmapFactory.decodeResource(getResources(), R.id.btn_addbs_img);
        ...
        if (v.getId() == R.id.btn_addbs_cancle) {
            Intent intent = new Intent(getApplicationContext(), Activity_book_list.class);
            startActivity(intent);
            finish();
        }
        if (v.getId() == R.id.btn_addbs_img) {
